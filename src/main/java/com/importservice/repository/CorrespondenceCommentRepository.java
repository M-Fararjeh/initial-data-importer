...
public interface CorrespondenceCommentRepository extends JpaRepository<CorrespondenceComment, String> {
    ...
    @Query(value = "SELECT " +
                   ...
                   "LEFT JOIN correspondences c ON cc.doc_guid = c.guid " +
                   "LEFT JOIN incoming_correspondence_migrations icm ON cc.doc_guid = icm.correspondence_guid " +
                   "WHERE (:status IS NULL OR cc.migrate_status = :status) " +
     ...
     */
    @Query(value = "SELECT COUNT(*) FROM correspondence_comments WHERE migrate_status = :status", 
           nativeQuery = true)
    ...
    @Query(value = "SELECT cc.* FROM correspondence_comments cc " +
                   "LEFT JOIN incoming_correspondence_migrations icm ON cc.doc_guid = icm.correspondence_guid " +
                   "WHERE (cc.migrate_status = 'PENDING' OR (cc.migrate_status = 'FAILED' AND cc.retry_count < 3)) " +
     ...
     */
    @Query("SELECT cc FROM CorrespondenceComment cc WHERE cc.migrateStatus IN :statuses ORDER BY cc.lastModifiedDate DESC")
