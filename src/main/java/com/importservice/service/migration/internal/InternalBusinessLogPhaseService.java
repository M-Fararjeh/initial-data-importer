...
/**
 * Service for Internal Phase 5: Business Log
 * Processes business logic and workflows for internal correspondences
 */
@Service
public class InternalBusinessLogPhaseService {
    
    private static final Logger logger = LoggerFactory.getLogger(InternalBusinessLogPhaseService.class);
     ...
     * Processes business logic for internal correspondences
     */
    @Transactional(readOnly = false, timeout = 600)
    public ImportResponseDto executeBusinessLogPhase() {
        logger.info("Starting Internal Phase 5: Business Log");
        ...
        try {
            // Get business logs for internal correspondences (non-assignment transactions)
            List<CorrespondenceTransaction> businessLogs = getInternalBusinessLogsNeedingProcessing();
            
            for (CorrespondenceTransaction businessLog : businessLogs) {
                try {
                    boolean success = processInternalBusinessLog(businessLog);
                    if (success) {
                        successfulImports++;
                    ...
                    failedImports++;
                    String errorMsg = "Error processing internal business log " + businessLog.getGuid() + ": " + e.getMessage();
                    errors.add(errorMsg);
                    businessLog.setMigrateStatus("FAILED");
                    businessLog.setRetryCount(businessLog.getRetryCount() + 1);
                    transactionRepository.save(businessLog);
            ...
            String message = String.format("Internal Phase 5 completed. Processed: %d, Failed: %d", 
            ...
            logger.error("Error in Internal Phase 5: Business Log", e);
            return phaseService.createResponse("ERROR", "Internal Phase 5 failed: " + e.getMessage(), 
    ...
    public ImportResponseDto executeBusinessLogForSpecific(List<String> transactionGuids) {
        ...
        String status = phaseService.determineFinalStatus(successfulImports, failedImports);
        String message = String.format("Specific internal business log completed. Processed: %d, Failed: %d", 
                                     successfulImports, failedImports);
    ...
    private List<CorrespondenceTransaction> getInternalBusinessLogsNeedingProcessing() {
        // Use the new method that filters by internal correspondence type
        return transactionRepository.findInternalBusinessLogsNeedingProcessing();
    }
    ...
    private boolean processInternalBusinessLogForTransaction(String transactionGuid) {
        try {
            ...
            boolean success = processInternalBusinessLog(transaction);
            ...
            logger.error("Error processing internal business log for: {}", transactionGuid, e);
            return false;
        }
    }
    
    /**
     * Processes a single internal business log transaction
     */
    private boolean processInternalBusinessLog(CorrespondenceTransaction transaction) {
        try {
            ...
            if (!migrationOpt.isPresent() || migrationOpt.get().getCreatedDocumentId() == null) {
                logger.error("No created document ID found for internal correspondence: {}", transaction.getDocGuid());
                return false;
            }
            
            String documentId = migrationOpt.get().getCreatedDocumentId();
            
            // Use administrator as creator for business log as specified in requirements
            String fromUser = "cts_admin";
            ...
            logger.error("Error processing internal business log: {}", transaction.getGuid(), e);
            return false;
        }
    }
    
    /**
     * Gets internal business log migrations with pagination and search
     */
    @Transactional(readOnly = true, timeout = 60)
    public Map<String, Object> getInternalBusinessLogMigrations(int page, int size, String status, String search) {
        try {
            Pageable pageable = PageRequest.of(page, size);
            
            // Use internal-specific business log query methods
            ...
            if ((status != null && !"all".equals(status)) || (search != null && !search.trim().isEmpty())) {
                ...
                businessLogPage = transactionRepository.findInternalBusinessLogMigrationsWithSearchAndPagination(
                    statusParam, searchParam, pageable);
            } else {
                businessLogPage = transactionRepository.findInternalBusinessLogMigrationsWithPagination(pageable);
            }
            ...
            logger.error("Error getting internal business log migrations", e);
